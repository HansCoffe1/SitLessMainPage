"use strict";
//主要是作为一个模板头文件一样的存在提供给各funs使用的中间函数
$(() => {
    fun_id = parseInt(location.href.match(/fun(\d*)/i)[1]);
    // console.log(fun_id);
    detect_css();
...
//由于有不止一个工具需要使用到随机数，所以直接在fun_common提供随机数中间函数
...
function help_click(bid = "help") {
    ...
    if (obj.val() == '收起帮助') {
        obj.val('展开帮助');
        $('.fun_help').slideUp(500);
    } else {
        obj.val('收起帮助');
        $('.fun_help').slideDown(500);
    }
    setTimeout(() => {
        set_footer_position();
    }, 501);
